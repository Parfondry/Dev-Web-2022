        ...
        if (ProfilUser.length !== 0){
            if (Profil.length !== 0){
                ...
                <form onSubmit={ModifProfil}>
                    ...
                    { currentShow2 ? <div><label name={'newMail'} >Nouveau Mail : </label><input name={'newMail'} type={'text'} placeholder='example@email.com'></input></div> : null }
                    <label>mdp : ********</label> <button onClick={(e) => {e.preventDefault(); setShow3(currentShow3 => !currentShow3)}}>Modifier</button><br/>
                    { currentShow3 ? <div><label name={'newMdp'} >Nouveau mdp : </label><input name={'newMdp'} type={'password'} placeholder='mdp'></input></div> : null }
                    <label>Date de naissances : {Profil[0].Birth}</label> <button onClick={(e) => {e.preventDefault(); setShow4(currentShow4 => !currentShow4)}}>Modifier</button><br/>
                    { currentShow4 ? <div><label name={'newDate'} >Nouvelle date : </label><input name={'newDate'} type={'date'}></input></div> : null }
